...
function loginWithIntra() {
    ...
    if (useRealAuth) {
        ...
        const clientId = "TU_CLIENT_ID_REAL"; // Reemplaza con tu ID real
        const redirectUri = encodeURIComponent("https://arr4str2.relixr.io/backend/api/oauth/callback"); // Debe coincidir con el backend
        ...
        console.log("🟢 Simulando login con intra...");
...
function checkIntraLogin() {
    ...
    if (useRealAuth) {
        ...
        // Extraer tokens desde la URL (ya sea search o hash)
        const urlParams = new URLSearchParams(window.location.search);
        const hashParams = new URLSearchParams(window.location.hash.split('?')[1]); // Maneja tokens en el hash
        const accessToken = urlParams.get('access') || hashParams.get('access');
        ...
        if (accessToken && refreshToken) {
            ...
            console.log("⚠️ No se encontraron tokens en la URL. Comprobando cookies...");
        ...
        console.log("🟢 Simulando autenticación con intra...");

        setTimeout(() => {
            console.log("✅ Simulación: usuario autenticado.");
            localStorage.setItem("access_token", "fakeAccessToken42");
            localStorage.setItem("refresh_token", "fakeRefreshToken42");
...
function autenticarUsuario() {
    ...
    if (getCookie("2fa_required") === "true") {
        console.log("⚠️ Se requiere 2FA para este usuario.");
        send2FARequest();
    }
}
...
function handleSignUp() {
    ...
    if (useRealAuth) {
        ...
        setTimeout(() => {
            console.log(`✅ Simulación: Usuario ${username} registrado.`);
            isLoggedIn = true;
            localStorage.setItem("access_token", "fakeAccessToken42");
            localStorage.setItem("refresh_token", "fakeRefreshToken42");
...
//  Envío de datos de Sign up al backend
function sendSignUpRequest(username, email, password) {
    console.log(`🔵 Enviando Sign Up: Username: ${username}, Email: ${email}`);
...
function handleSignIn() {
    ...
    console.log("✅ Datos validados. Iniciando sesión...");
...
function sendSignInRequest(email, password) {
    console.log(`🔵 Enviando Sign In: Email: ${email}, Password: [OCULTO]`);
    ...
    .then(data => {
        if (data.access) {
            ...
            if (data.twoFARequired) {
                console.log("⚠️ 2FA requerido. Solicitando autenticación...");
                send2FARequest();
            }
        } else {
...
function logoutUser() {
    ...
    isLoggedIn = false; // Desactiva la sesión antes de actualizar la UI

    // Intentar cerrar sesión en el backend si es posible
    ...
    .catch(error => console.error("❌ Error al cerrar sesión en el backend:", error))
    .finally(() => {
        ...
        /*
        // ❌ SOLO DESCOMENTAR SI EL MENÚ DE LOGIN NO SE MUESTRA DESPUÉS DEL LOGOUT
...
// Elimina cookies de sesión (tanto frontend como backend)
function eliminarCookiesSesion() {
    console.log("Eliminando cookies de sesión...");

    const cookies = ["access_token", "refresh_token", "2fa_required", "csrftoken", "sessionid"];
    
    cookies.forEach(cookie => {
        document.cookie = `${cookie}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; secure; SameSite=None;`;
        document.cookie = `${cookie}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/backend/; secure; SameSite=None;`;
        document.cookie = `${cookie}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=arr4str2.relixr.io; secure; SameSite=None;`;
    });
